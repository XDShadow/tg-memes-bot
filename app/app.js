...
app.hears(/пидор(\s|$)|пидр(\s|$)|pidor(\s|$)|pidr(\s|$)|гей(\s|$)|homo(\s|$)/i, (ctx) => {
    const replies = ['Самый пидорский пидор из всех, что я видел!', 'Рогов у него вроде нет, так что выбор не велик! 🐔', 'Сам пидор! 🐔', 'Ты че, ахуел, пидор?! 🐔', 'Кто пидор? Я не пидор!', 'Че сразу пидор то?', 'Да, тот еще пидор!🐔', 'Да всего-то раз было!', 'Один раз не пидорас!']
    ctx.reply(replies[Math.floor(Math.random() * replies.length)]);
});
